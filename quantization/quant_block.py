import torch as th # From openaimodule
...
class QuantSMVMatMul(BaseQuantBlock):
    def __init__(self, act_quant_params: dict = {}, sm_abit=8):
        super().__init__(act_quant_params)
        # New
        self.use_act_quant = False
        self.act_quantizer_v = Quantizer(**act_quant_params)

        # Modify act_quant_params for weight
        act_quant_params_w = act_quant_params.copy()
        act_quant_params_w['n_bits'] = sm_abit
        act_quant_params_w['symmetric'] = False # Full range utilization w != model weight, w = softmax(QK^T)
...
class QuantAttentionBlock(BaseQuantBlock):
    # Nothing modified except copy
    def __init__(self, attn: AttentionBlock, act_quant_params: dict = {}):
        super().__init__(act_quant_params)
        # From original AttentionBlock
...
def cross_attn_forward(self, x, context=None, mask=None):
    # cross_attin_forward not in original
    h = self.heads
...
def get_quantized(quant_act=False):
    # Default quantized blocks
    quantized = {
    ...
    # Only changes when activation quantization is True/False
    if quant_act:
        quantized[QKMatMul] = QuantQKMatMul
        quantized[SMVMatMul] = QuantSMVMatMul
    else:
        quantized[AttentionBlock] = QuantAttentionBlock
    return quantized